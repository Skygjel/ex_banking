defmodule StressTest do
  ...
  defp ddos_test_helper(function_to_use, user, currency, amount, process) do
    for _ <- 1..10 do
      case function_to_use.(user, amount, currency) do
        {:error, :timeout} ->
          send(process, {:error, :timeout})

        {:error, :too_many_requests_to_user} ->
          send(process, {:error, :too_many_requests_to_user})
  ...
  def compleation_loop(expected_compleations, timeouts, too_many_requests) do
    receive do
      {:error, :timeout} ->
        compleation_loop(expected_compleations, timeouts + 1, too_many_requests)

      {:error, :too_many_requests_to_user} ->
        compleation_loop(expected_compleations, timeouts, too_many_requests + 1)

      :done ->
        compleation_loop(expected_compleations - 1, timeouts, too_many_requests)
    end
  end
end
